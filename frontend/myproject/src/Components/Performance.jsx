...
const Performance = () => {
          ...
          if (isOverallView) {
            ...
            const courseData = response.data.data.find((result) => {
              ...
              return (
                ...
                courseIdStr.toLowerCase().startsWith(subjectStr.toLowerCase() + "-") ||
  ...
  const getImprovementTrend = (attempts) => {
    ...
    const firstAvg =
      firstHalf.reduce((sum, a) => sum + (a.score || 0), 0) / (firstHalf.length || 1);
    const secondAvg =
      secondHalf.reduce((sum, a) => sum + (a.score || 0), 0) / (secondHalf.length || 1);
  ...
  const globalStats = React.useMemo(() => {
    ...
    const allAttempts = allExamData.flatMap(course =>
    ...
    const passingRate = totalAttempts > 0 ? Math.round((passedAttempts / totalAttempts) * 100) : 0;
    const subjectsCleared = new Set(allAttempts.filter(a => a.passed).map(a => a.courseId)).size;
    return {
      allAttempts,
      totalAttempts,
      avgScore,
  ...
  const currentScore = isOverallView && globalStats ? globalStats.avgScore : (examData?.score || 0);
  const currentAttempts = isOverallView && globalStats ? [...globalStats.allAttempts].reverse() : (examData?.examAttempts || []);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 animate-pulse font-medium">Loading performance data...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center p-8">
          ...
          <p className="text-gray-500 mb-6">You haven't attempted any exams for this course yet.</p>
          <button onClick={() => navigate(-1)} className="px-6 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-colors">Go Back</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center p-8">
          <FaBookOpen className="text-6xl text-gray-300 mx-auto mb-4" />
          <h2 className="text-2xl font-bold text-gray-900 mb-2">No Performance Data</h2>
          <p className="text-gray-500 mb-6">You haven't attempted any exams yet.</p>
          <button onClick={() => navigate(-1)} className="px-6 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-colors">Go Back</button>
        </div>
      </div>
    );
      ...
      <div className="max-w-6xl mx-auto relative z-10 mt-12">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">
            {isOverallView ? "Universal Knowledge Explorer" : "Performance Analytics"}
          </h1>
          <p className="text-gray-600 text-lg">
            {isOverallView ? "A comprehensive overview of all your learning achievements" : `${subject} - Detailed Exam Results`}
          </p>
        </div>
                ...
                {globalStats?.allAttempts?.map((attempt, index) => (
                  <div
                    ...
                    onClick={() => {
                      ...
                      const element = document.getElementById('question-analysis');
                  ...
                  >
                    <div className="flex items-center gap-4 text-left rounded-xl transition">
                      ...
                      <div>
                        <h4 className={`font-bold text-lg transition-colors ${selectedAttempt?.attemptId === attempt.attemptId ? "text-blue-600" : "text-gray-800 group-hover:text-blue-600"
                          }`}>
                    ...
                    <div className="flex items-center gap-6">
                      <div className="flex flex-col text-right">
                        <p className={`text-xs font-bold uppercase tracking-wider ${attempt.passed ? "text-green-600" : "text-red-500"}`}>
                          {attempt.passed ? "✅ Passed" : "📚 Still need to be Focused"}
                        </p>
                      ...
                      <div className={`p-2 transition-colors ${selectedAttempt?.attemptId === attempt.attemptId ? "text-blue-600" : "text-gray-300 group-hover:text-blue-600"
                        }`}>
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-8 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-gray-50 border border-gray-200 rounded-xl p-6">
                  <div className="text-3xl font-bold text-gray-900 mb-2">{globalStats.totalAttempts}</div>
                  <div className="text-gray-500 text-sm">Total Attempts</div>
                </div>
                <div className="bg-gray-50 border border-gray-200 rounded-xl p-6">
                  <div className="text-3xl font-bold text-gray-900 mb-2">{globalStats.avgScore}%</div>
                  <div className="text-gray-500 text-sm">Average Score</div>
                </div>
                <div className="bg-gray-50 border border-gray-200 rounded-xl p-6">
                  <div className="text-3xl font-bold text-gray-900 mb-2">{globalStats.passingRate}%</div>
                  <div className="text-gray-500 text-sm">Passing Rate</div>
                </div>
                <div className="bg-gray-50 border border-gray-200 rounded-xl p-6">
                  <div className="text-3xl font-bold text-gray-900 mb-2">{globalStats.subjectsCleared}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
              <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  <PerformanceIcon className={`text-3xl ${performanceLevel.color}`} />
                  <span className={`text-xs font-bold px-2 py-1 rounded-full ${performanceLevel.bg} ${performanceLevel.border} ${performanceLevel.color}`}>
                    {performanceLevel.level}
                  </span>
                </div>
                <div className="text-3xl font-bold text-gray-900 mb-2">{currentScore}%</div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-xs font-bold px-2 py-1 rounded-full bg-blue-50 border border-blue-200 text-blue-600">Active</span>
                </div>
                <div className="text-3xl font-bold text-gray-900 mb-2">{currentAttempts.length}</div>
                <div className="text-gray-500 text-sm">Total Attempts</div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-xs font-bold px-2 py-1 rounded-full bg-orange-50 border border-orange-200 text-orange-600">Hot!</span>
                </div>
                <div className="text-3xl font-bold text-gray-900 mb-2">{streakInfo.current}</div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-xs font-bold px-2 py-1 rounded-full bg-yellow-50 border border-yellow-200 text-yellow-600">Record</span>
                </div>
                <div className="text-3xl font-bold text-gray-900 mb-2">{streakInfo.best}</div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 mb-12 shadow-sm">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <FaChartLine className={`text-2xl ${improvementTrend.trend === "up" ? "text-green-600" : improvementTrend.trend === "down" ? "text-red-600" : "text-yellow-600"}`} />
                  <div>
                    <h3 className="text-lg font-bold text-gray-900">Recent Performance</h3>
                    <p className="text-gray-500 text-sm">Based on last 5 attempts</p>
                  </div>
                </div>
                <div className="text-right">
                  <div className={`text-2xl font-bold ${improvementTrend.trend === "up" ? "text-green-600" : improvementTrend.trend === "down" ? "text-red-600" : "text-yellow-600"}`}>
                    {improvementTrend.trend === "up" ? "↑" : improvementTrend.trend === "down" ? "↓" : "→"} {Math.abs(improvementTrend.change)}%
                  </div>
                  <div className="text-gray-500 text-sm">{improvementTrend.trend === "up" ? "Improving" : improvementTrend.trend === "down" ? "Declining" : "Stable"}</div>
                </div>
              </div>
            </div>
          </>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="text-left py-4 px-4 font-bold text-gray-900">Chapter Name</th>
                    <th className="text-center py-4 px-4 font-bold text-gray-900">Best Score</th>
                    <th className="text-center py-4 px-4 font-bold text-gray-900">Attempts</th>
                ...
                <tbody>
                  {allExamData.map((course, idx) => {
                    ...
                    const totalAttempts = course.attempts || (course.examAttempts ? course.examAttempts.length : 0);
                    const passed = course.passed || false;
                    return (
                      <tr key={idx} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                        <td className="py-4 px-4 text-gray-900 font-medium">{course.courseName}</td>
                        <td className="text-center py-4 px-4">
                          <div className="flex items-center justify-center gap-2">
                            <span className="text-2xl">{getScoreEmoji(bestScore)}</span>
                            <span className="text-lg font-bold text-gray-900">{bestScore}%</span>
                          </div>
                        </td>
                        <td className="text-center py-4 px-4 text-gray-600 font-medium">{totalAttempts}</td>
                        <td className="text-center py-4 px-4">
                          <span className={`px-3 py-1 rounded-full text-xs font-bold ${passed ? "bg-green-50 border border-green-200 text-green-600" : "bg-yellow-50 border border-yellow-200 text-yellow-600"}`}>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-gray-900 font-bold">{attempt.score || 0}%</span>
                        </div>
                        <div className="text-gray-500 text-sm">{attempt.attemptDate ? new Date(attempt.attemptDate).toLocaleString() : "Recent"}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${attempt.passed ? "bg-green-50 border border-green-200 text-green-600" : "bg-red-50 border border-red-200 text-red-600"}`}>{attempt.passed ? "PASSED" : "FAILED"}</span>
                      {selectedAttempt?.attemptId === attempt.attemptId && <FaCheckCircle className="text-blue-600" />}
                    </div>
                  </div>
                </div>
              ))}
        ...
        {!isOverallView && selectedAttempt && selectedAttempt.answers && selectedAttempt.answers.length > 0 && (
          <div id="question-analysis" className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mt-12 scroll-mt-24">
            <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-3">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-green-600 font-bold">Correct Answers</span>
                </div>
              ...
              <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-red-600 font-bold">Incorrect Answers</span>
                </div>
                <div className="text-3xl font-bold text-gray-900">
                  {selectedAttempt.answers.filter((a) => !a.isCorrect).length}
                </div>
              ...
              {selectedAttempt.answers.map((answer, index) => (
                <div key={index} className="bg-gray-50 border border-gray-200 rounded-xl overflow-hidden">
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${answer.isCorrect ? "bg-green-50" : "bg-red-50"}`}>
                          {answer.isCorrect ? <FaCheckCircle className="text-green-600 text-sm" /> : <FaTimesCircle className="text-red-600 text-sm" />}
                        </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-400">{expandedQuestions.has(index) ? "−" : "+"}</span>
                      </div>
                    </div>
                  ...
                  {expandedQuestions.has(index) && (
                    <div className="px-4 pb-4 border-t border-gray-200">
                      <div className="pt-4 space-y-4">
                        <div>
                          <p className="text-gray-600 text-sm mb-2 font-medium">Question:</p>
                          <p className="text-gray-900 bg-gray-50 p-3 rounded-lg">{answer.question}</p>
                        </div>
                        <div className="p-4 rounded-lg border bg-gray-50 border-gray-200">
                          <p className="text-xs uppercase font-bold text-gray-600 mb-2">Your Answer</p>
                          ...
                          <p className={`text-sm mt-2 font-bold ${answer.isCorrect ? "text-green-600" : "text-red-600"}`}>
                            {answer.isCorrect ? "✓ Correct" : "✗ Incorrect"}
                          </p>
                        </div>
                                ...
                                >
                                  <div className="flex items-center justify-between">
                                    ...
                                    {optIndex === answer.selectedOption && (
                                      <span className="text-blue-600 text-xs font-bold bg-blue-50 px-2 py-1 rounded">YOUR ANSWER</span>
                                    )}
                                  </div>
                                </div>
                              ))
                            ) : (
                              <div className="text-gray-600 text-sm p-4 bg-gray-50 rounded-lg border border-gray-200">Options not available</div>
                            )}
                          </div>
                        </div>
                      </div>

...
apiClient.interceptors.response.use(
  (response) => {
    ...
    const requestKey = `${response.config.method?.toUpperCase()}_${response.config.url}_${JSON.stringify(response.config.params || {})}_${JSON.stringify(response.config.data || {})}`;
  ...
  (error) => {
    ...
    if (error.config) {
      const requestKey = `${error.config.method?.toUpperCase()}_${error.config.url}_${JSON.stringify(error.config.params || {})}_${JSON.stringify(error.config.data || {})}`;
      pendingRequests.delete(requestKey);
    }
